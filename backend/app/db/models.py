...
class LiveChallenge(Base):
    """Judge-controlled evidence injection record used by the Grand Finals challenge mode.

    This intentionally stores analysis results separately from canonical decisions so arbitrary
    live inputs can be tested without silently mutating approved enterprise policy.
    """
    __tablename__ = "live_challenges"
    id = Column(Integer, primary_key=True)
    challenge_ref = Column(String(40), unique=True, index=True, nullable=False)
    source = Column(String(80), nullable=False)
    title = Column(String(180), nullable=False)
    body = Column(Text, nullable=False)
    inferred_rule_key = Column(String(80), index=True, nullable=False)
    inferred_claim = Column(Text, nullable=False)
    evidence_ref = Column(String(60), index=True, nullable=False)
    conflict_ref = Column(String(40), index=True, nullable=True)
    verdict = Column(String(40), index=True, nullable=False)
    confidence = Column(Float, nullable=False)
    blast_radius = Column(Integer, default=0, nullable=False)
    status = Column(String(40), default="quarantined", index=True, nullable=False)
    analysis_json = Column(Text, default="{}", nullable=False)
    created_by = Column(String(160), nullable=False)
    created_at = Column(DateTime(timezone=True), default=now_utc)
