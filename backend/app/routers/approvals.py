...
def approve(ref: str, body: ApprovalDecisionRequest, db: Session = Depends(get_db), user: User = Depends(require_roles("manager", "compliance_manager", "product_owner"))):
    ...
    if gate.get("status") != "PASS":
        ...
        raise HTTPException(409, detail={
            "message":"Decision publication blocked by JurisTwin Governance Gate",
    ...
    if not contract:
        contract = DecisionContract(
            ...
            approved_by="Product Owner + Functional Lead", effective_at=datetime(2026, 7, 24, tzinfo=timezone.utc), supersedes="Payslips-only requirement (v3.0)",
            affected_json=dumps({"applications": 27, "rejected_cases": 1, "qa_tests": 8, "tasks": 3, "documents": 2, "officers_notified": 4}),
    ...
    append_entry(db, "DECISION_APPROVED", user.email, {"approval_ref": a.approval_ref, "decision_ref": decision_ref, "selected_option": "C", "governance_gate_score": gate.get("score"), "governance_gate":"PASS"}, decision_ref)
    append_entry(db, "DECISION_PROPAGATED", "sentinel-orchestrator", {"applications_reviewed": len(cases), "rejected_cases_flagged": 1, "qa_tests_updated": 8, "documents_superseded": 3, "officers_notified": 4}, decision_ref)
    db.commit(); db.refresh(a); db.refresh(contract)
    return {"approval": serialize(a), "decision_contract": serialize_contract(contract), "propagation": {"cases": len(cases), "rejected_cases": 1, "qa_tests": 8, "documents": 3, "officers_notified": 4}}
