(() => {
  ...
  // v7.0 championship finals UX: live story follows the sidebar — Ask → Control → Explain → Compare → Human Gate → Challenge → Protect.
  ...
  const state = {
    ...
    lastPublishedDecision: null,
    boundaryEvent: null,
    lastSecurityAction: null,
  ...
  const META = {
    ...
    controls:['Management Controls','Management defines the evidence boundary.'],
    ...
    assurance:['Safe to Publish?','AI recommends. Humans retain authority.'],
  ...
  async function api(path, options={}){
    ...
    for (let attempt=0; attempt<maxAttempts; attempt++){
      ...
      try {
        const r = await fetch(`/api${path}`, {...options, method, headers, signal:controller.signal, cache:method==='GET'?'no-store':options.cache});
        clearTimeout(timer);
        if (r.status === 401 && path !== '/auth/login' && path !== '/auth/session') {
          logout(false);
          throw new Error('Session expired. Sign in again.');
        }
        // One safe retry for transient server failures on idempotent GET requests only.
        if (method==='GET' && attempt+1<maxAttempts && [502,503,504].includes(r.status)){
        ...
        clearTimeout(timer);
        lastError=e;
        if (method==='GET' && attempt+1<maxAttempts && (e.name==='AbortError'||e instanceof TypeError)){
          await new Promise(resolve=>setTimeout(resolve,120));
          continue;
        }
        if (e.name === 'AbortError') throw new Error('Request timed out safely. Please retry.');
        throw e;
      }
    }
    throw lastError || new Error('Request failed safely.');
  }
  ...
  async function resetDemo(){
    try {
      ...
      closeSheet(); state.sim=null; state.lastChallenge=null; state.overviewAnswer=null; state.overviewRole='manager'; state.selectedOption='C'; state.graphPositions={}; state.assurance=null; state.readiness=null; state.security=null; state.managerControls=null; state.lastPublishedDecision=null; state.boundaryEvent=null; state.lastSecurityAction=null; state.challengeStartedAt=null; state.decisionRefs={}; await refreshCore(); renderShell(); status('Finals scenario restored','ok');
    } catch(e) { status(e.message,'error',3200); }
  }
  ...
  function renderManagementControls(){
    ...
    if(!d)return `<div class="page"><section class="page-hero reveal"><div><span class="eyebrow">MANAGEMENT CONTROL PLANE</span><h2>Who controls what JurisTwin is allowed to trust?</h2><p>Management — not the AI.</p></div><div class="hero-actions"><button class="btn primary" id="refreshManagerControls">Load controls</button></div></section><section class="surface manager-empty reveal"><div><div class="empty-icon">${icon('shield',22)}</div><h3>Loading the evidence boundary…</h3></div></section></div>`;
    const security=state.security||{}, sources=security.source_policies||[];
    const byKey=Object.fromEntries(sources.map(x=>[x.key,x]));
    const sourceKeys=['sharepoint','outlook','teams','customer_core'];
    const sourceFallback={
      sharepoint:{key:'sharepoint',name:'Approved Policies / SOPs',status:'connected',retrieval_enabled:true,policy_authority_enabled:true,scope_label:'Approved policy/SOP libraries'},
      outlook:{key:'outlook',name:'Official Management Email',status:'connected',retrieval_enabled:true,policy_authority_enabled:true,scope_label:'Official management/policy mailboxes only'},
      teams:{key:'teams',name:'Approved Teams Channels',status:'connected',retrieval_enabled:true,policy_authority_enabled:false,scope_label:'Approved group/channel chats only'},
      customer_core:{key:'customer_core',name:'Customer Core',status:'connected',retrieval_enabled:true,policy_authority_enabled:false,scope_label:'Operational impact only'},
    };
    const sourceCards=sourceKeys.map(key=>byKey[key]||sourceFallback[key]);
    const assuranceReady=Number(d.passed||0)===Number(d.total||0);
    const sourceCard=(x)=>`<article class="control-source-card ${!x.retrieval_enabled||x.status==='inactive'?'source-off':''}"><div class="control-source-card-head"><div><small>${esc(x.key==='customer_core'?'IMPACT DATA':x.policy_authority_enabled?'GOVERNED SOURCE':'SUPPORTING SOURCE')}</small><b>${esc(x.name)}</b></div><button type="button" class="source-live-switch ${x.retrieval_enabled&&x.status!=='inactive'?'on':'off'}" data-source-toggle="${esc(x.key)}" data-source-enabled="${x.retrieval_enabled&&x.status!=='inactive'?'true':'false'}" aria-label="Toggle ${esc(x.name)} retrieval"><i></i>${x.retrieval_enabled&&x.status!=='inactive'?'ON':'OFF'}</button></div><p>${esc(brief(sourceScopeSummary(x),88))}</p><footer><span class="chip ${x.policy_authority_enabled?'green':'cyan'}">${x.policy_authority_enabled?'CAN ESTABLISH POLICY':x.key==='customer_core'?'IMPACT ONLY':'REFERENCE ONLY'}</span><button class="btn quiet" data-source-config="${esc(x.key)}">Scope</button></footer></article>`;
    return `<div class="page controls-page finals-controls-page compact-page">
      <section class="page-hero controls-hero reveal"><div><span class="eyebrow">MANAGEMENT CONTROL PLANE</span><h2>Who controls what JurisTwin is allowed to trust?</h2><p>The evidence boundary is explicit, adjustable and enforced before an answer is generated.</p></div><div class="hero-actions"><span class="chip green">MANAGEMENT · NOT THE AI</span><button class="btn" id="refreshManagerControls">Re-check</button></div></section>
      <section class="controls-command-grid reveal">
        <article class="surface authority-ladder-panel"><div class="section-head compact"><div><span class="eyebrow">SOURCE AUTHORITY</span><h3>Authority before frequency.</h3></div><span class="chip green">ACTIVE RULE</span></div><div class="authority-ladder">
          <div class="authority-tier top"><span>1</span><div><b>Canonical decision / governed policy</b><small>Published Decision Contract · approved policy owner</small></div><em>HIGHEST</em></div>
          <div class="authority-tier"><span>2</span><div><b>Official management communication</b><small>Approved Outlook / policy-owner instruction</small></div><em>OFFICIAL</em></div>
          <div class="authority-tier"><span>3</span><div><b>Controlled operational guidance</b><small>Approved Teams channels · training · controlled SOP context</small></div><em>SUPPORT</em></div>
          <div class="authority-tier blocked"><span>×</span><div><b>Private or casual content</b><small>Personal DMs and casual email cannot establish company policy</small></div><em>ZERO AUTHORITY</em></div>
        </div><div class="authority-rule"><b>10 informal messages cannot outvote 1 approved decision.</b><span>Majority is used only when equally authoritative approved sources disagree.</span></div></article>
        <article class="surface evidence-boundary-panel"><div class="section-head compact"><div><span class="eyebrow">LIVE EVIDENCE BOUNDARY</span><h3>Management chooses what participates.</h3></div><span class="chip ${security.source_policies?'green':'cyan'}">${security.source_policies?'LIVE':'LOADING'}</span></div><div class="control-source-grid">${sourceCards.map(sourceCard).join('')}</div>${state.boundaryEvent?`<div class="boundary-live-event">${icon('spark',16)}<div><small>BOUNDARY UPDATED · ${esc(state.boundaryEvent.at)}</small><b>${esc(state.boundaryEvent.name)} is now ${state.boundaryEvent.enabled?'IN':'OUT OF'} governed retrieval.</b><span>The next answer will recompute against this exact boundary.</span></div><button class="btn primary" id="rerunBoundary">Re-run same question ${icon('arrow',12)}</button></div>`:`<div class="boundary-demo-note">${icon('spark',15)}<span><b>Live control:</b> toggle a source here. JurisTwin applies the new evidence boundary to the very next answer.</span></div>`}</article>
      </section>
      <section class="control-rule-grid reveal"><article class="surface"><span>01</span>${icon('shield',18)}<div><b>Authority before frequency</b><p>Highest approved authority wins. Same-tier majority is only a fallback.</p></div></article><article class="surface"><span>02</span>${icon('lock',18)}<div><b>Privacy before retrieval</b><p>Private and ineligible evidence is removed before relevance scoring can influence an answer.</p></div></article><article class="surface"><span>03</span>${icon('check',18)}<div><b>Human before publication</b><p>The AI may recommend, but its publication authority is always zero.</p></div></article></section>
      <section class="surface control-assurance-bar reveal"><div><span class="chip ${assuranceReady?'green':'amber'}">${assuranceReady?'ALL CONTROLS PASS':'REVIEW'}</span><b>${fmt(d.passed)}/${fmt(d.total)} live controls verified</b><small>Backend-derived checks: source scope, authority, privacy, export RBAC, protected transfer, audit and real-time recomputation.</small></div><div><button class="btn" id="controlsAssuranceDetails">View technical evidence</button><button class="btn primary" id="controlsToConflict">See a real disagreement ${icon('arrow',12)}</button></div></section>
      <details class="surface controls-evidence-details reveal"><summary>Technical assurance evidence · ${fmt(d.passed)}/${fmt(d.total)} checks</summary><div class="assurance-evidence-list">${(d.controls||[]).map((c,i)=>`<div><span>${String(i+1).padStart(2,'0')}</span><b>${esc(c.title)}</b><small>${esc(c.proof||'Control active')}</small><em class="${c.status==='PASS'?'pass':'review'}">${esc(c.status)}</em></div>`).join('')}</div></details>
    </div>`;
  }
  ...
  function overviewAnswerMarkup(){
    ...
    if(!r)return `<div class="overview-answer-placeholder manager-placeholder compact-placeholder"><div><b>Ask a question. Get one answer + source.</b><span>Then switch role to prove the same question is permission-aware.</span></div><div class="source-mini-row"><span>${icon('check',12)} Official email</span><span>${icon('check',12)} Approved Teams</span><span>${icon('check',12)} Policies</span><span>${icon('lock',12)} DMs blocked</span></div></div>`;
    ...
    const conflict=state.conflicts.find(c=>c.conflict_ref===r.conflict_ref);
    const affected=Number(r.impact?.affected_cases??conflict?.affected_customers??0);
    const isConflict=r.status==='CONFLICT_PRESENT';
    const isIntern=String(r.role||state.overviewRole).toLowerCase()==='intern';
    const sourcesChecked=Number(r.synthesis?.sources_considered||sources.length||0);
    const impactHeadline=isIntern?'ACCESS BOUNDARY ENFORCED':isConflict?'CONFLICT DETECTED':'GOVERNED ANSWER READY';
    const impactCopy=isIntern?'Same question. Restricted evidence stays hidden for this role.':isConflict?`${affected} live customer cases are connected to this disagreement.`:'Current approved evidence is aligned.';
    return `<div class="overview-answer-result ${answerTone(r.status)} manager-answer compact-answer finals-answer">
      <div class="first-minute-impact ${isIntern?'role-proof':isConflict?'conflict':'verified'}"><div><small>LIVE DECISION</small><b>${impactHeadline}</b><span>${esc(impactCopy)}</span></div>${affected&&!isIntern?`<div class="first-minute-number"><strong>${fmt(affected)}</strong><span>CASES TRACED</span></div>`:''}</div>
      <div class="overview-answer-status"><span class="chip ${answerChip(r.status)}">${esc(managementStatus(r))}</span><span>${esc(cap(r.role||state.overviewRole))} view · recomputed now</span></div>
      ...
      <div class="manager-proof-grid compact-proof-grid">
        <article class="primary-proof"><div class="manager-proof-icon">${icon('check',16)}</div><small>MAIN OFFICIAL SOURCE</small><b>${esc(primary.title||r.source||'Approved source')}</b><span>${esc(primary.source||r.source||'Enterprise source')}</span></article>
        <article><div class="manager-proof-icon">${icon('shield',16)}</div><small>WHY IT WON</small><b>${esc(resolutionLabel(res.mode))}</b><span>${esc(brief(res.explanation||'Highest approved source wins.',72))}</span></article>
        <article><div class="manager-proof-icon">${icon('lock',16)}</div><small>PRIVACY</small><b>${fmt(res.eligible_count||sources.length)} used · ${fmt(excluded)} blocked</b><span>Private/casual sources excluded first</span></article>
      ...
      <div class="runtime-decision-trace" aria-label="Live decision pipeline"><span class="done">${icon('check',11)} Question received</span><i>→</i><span class="done">${icon('check',11)} ${fmt(sourcesChecked)} sources scoped</span><i>→</i><span class="done">${icon('check',11)} Authority resolved</span>${affected?`<i>→</i><span class="done">${icon('check',11)} ${fmt(affected)} cases traced</span>`:''}</div>
      <div class="role-proof-callout ${isIntern?'intern':''}"><div>${icon(isIntern?'lock':'shield',17)}<span><small>ROLE-BASED PROOF</small><b>${isIntern?'Restricted evidence is redacted server-side.':'Should every employee see the same evidence?'}</b></span></div><button class="btn ${isIntern?'':'primary'}" id="overviewRoleProof">${isIntern?'Return to Manager':'Preview same question as Intern'} ${icon('arrow',12)}</button></div>
      ${sourceMixMarkup(r,4)}
      ...
      <div class="answer-proof-row compact-actions"><span>${icon('shield',12)} Evidence-bound · model publish authority 0</span><div><button class="btn quiet" id="overviewSourcePolicy">Source rules</button><button class="btn quiet" id="overviewAIProof">Technical</button><button class="btn quiet" id="overviewRefresh">Refresh</button></div></div>
    </div>`;
  }
  ...
  async function publishDecision(){
    ...
    try {
      ...
      if(result.decision_contract?.decision_ref)state.decisionRefs[state.sim.conflict_ref]=result.decision_contract.decision_ref; state.lastPublishedDecision=result; state.assurance=null;state.readiness=null; closeSheet(); await refreshCore(); state.page='assurance'; renderShell(); setTimeout(loadAssurance,80); status(`Human-approved decision ${result.decision_contract?.decision_ref||'published'} · ${result.propagation?.cases||0} cases protected`,'ok',3800);
    } catch(e){ status(e.message,'error',3400); if(btn){btn.disabled=false;btn.textContent='Approve & publish';} }
  }

  function renderAssurance(){
    const a=state.assurance,r=state.readiness,c=selectedConflictObj();
    if(!a||!r)return `<div class="page compact-page"><section class="page-hero reveal"><div><span class="eyebrow">HUMAN GOVERNANCE GATE</span><h2>AI recommends. Humans decide.</h2><p>JurisTwin has zero authority to publish company policy.</p></div><div class="hero-actions"><button class="btn primary" id="loadAssurance">Run final checks ${icon('shield',13)}</button></div></section><section class="surface certificate-panel reveal manager-empty compact-empty"><div><div class="eyebrow">FINAL CHECK</div><h3>Prove the decision is safe before anyone publishes it.</h3><p>Evidence, customer impact, security and audit integrity are checked together.</p></div></section></div>`;
    ...
    const proposal=state.sim?`Option ${esc(state.selectedOption)} · ${esc(PITCH_OPTION_LABELS[state.selectedOption]||'Selected response')}`:'Run Compare Solutions first';
    const published=state.lastPublishedDecision;
    const protectedCount=published?.propagation?.cases ?? c?.affected_customers ?? 0;
    return `<div class="page compact-page assurance-page-finals"><section class="page-hero reveal"><div><span class="eyebrow">HUMAN GOVERNANCE GATE</span><h2>${resolved?'Human-approved. Now prove it.':'The AI stops here.'}</h2><p>${resolved?'The organisational rule changed only after an authorised human approved it.':'JurisTwin can recommend a response, but only an authorised human can change company truth.'}</p></div><div class="hero-actions"><span class="chip ${resolved?'green':'cyan'}">AI PUBLISH AUTHORITY · 0</span><button class="btn square" id="refreshAssurance" title="Refresh">${icon('reset',14)}</button></div></section>
      <section class="authority-zero-stage reveal ${resolved?'published':''}"><div><small>AI PUBLICATION AUTHORITY</small><strong>0<span>%</span></strong><p>Retrieve · classify · simulate · recommend</p></div><i>→</i><div class="human-authority"><small>HUMAN AUTHORITY</small><strong>${resolved?'APPROVED':'REQUIRED'}</strong><p>Only an authorised human may change company truth.</p></div></section>
      <section class="surface publish-command reveal ${resolved?'published':''}"><div class="publish-command-copy"><span class="chip ${resolved?'green':ready?'green':'amber'}">${resolved?'PUBLISHED BY HUMAN':ready?'READY FOR HUMAN APPROVAL':'REVIEW REQUIRED'}</span><small>PROPOSED RESPONSE</small><h3>${proposal}</h3><p>${resolved?`${fmt(protectedCount)} affected cases are now linked to the approved decision and audit trail.`:'Every required control must pass before the publish action is enabled.'}</p>${resolved&&published?.decision_contract?`<div class="published-proof-mini"><span><small>DECISION</small><b>${esc(published.decision_contract.decision_ref||'Published')}</b></span><span><small>APPROVER</small><b>${esc(published.approval?.approved_by||state.user?.email||'Authorised human')}</b></span><span><small>CASES</small><b>${fmt(protectedCount)}</b></span><span><small>INTEGRITY</small><b>READY TO VERIFY</b></span></div>`:''}</div><div class="publish-authority-boundary"><div><span>AI</span><b>Recommend</b><em>Allowed</em></div><i>→</i><div class="human"><span>HUMAN</span><b>Publish</b><em>${resolved?'Completed':'Required'}</em></div></div><div class="publish-command-action">${resolved?`<button class="btn primary" id="assuranceVerify">Verify decision record ${icon('arrow',12)}</button><button class="btn" id="assuranceNextChallenge">Challenge with new evidence</button>`:state.sim?`<button class="btn primary" id="assuranceApprove" ${ready&&state.selectedOption==='C'?'':'disabled'}>${ready?'Approve & publish':'Complete checks first'}</button>`:`<button class="btn primary" id="assuranceCompare">Compare solutions first ${icon('arrow',12)}</button>`}</div></section>
      <section class="assurance-live-grid reveal"><article class="surface assurance-gate-card"><div class="gate-score-ring"><strong>${gatePassed}</strong><span>/${gateChecks.length}</span></div><div><span class="eyebrow">GOVERNANCE GATE</span><h3>${gate.status==='PASS'?'Every required check passed.':'A required check is blocking publication.'}</h3><div class="gate-check-pills">${gateChecks.map(x=>`<span class="${x.ok?'ok':'bad'}">${x.ok?'✓':'!'} ${esc(({'Canonical authority':'Official owner','Impact is explainable':'Affected cases','Decision robustness':'Robust response','Ledger integrity':'Audit integrity','No unresolved critical security block':'Security'})[x.label]||x.label)}</span>`).join('')}</div></div></article><article class="surface assurance-readiness-card"><span class="eyebrow">SYSTEM READINESS</span><strong>${r.score}<small>/100</small></strong><b>${esc(r.status||'CHECK')}</b><p>Database, RBAC, evidence, ledger and safety invariants are checked before publication.</p></article></section>
      <details class="technical-details page-technical reveal"><summary>Technical assurance proof</summary><p>Runtime success ${Number(tel.success_rate_pct||0).toFixed(0)}% · p95 ${Number(tel.latency_ms?.p95||0).toFixed(1)} ms · governance gate ${gate.score||0}/100 · invariant status ${esc(inv.status||'—')}.</p><div class="feature-actions"><button class="btn" id="proofPack">Audit proof</button><button class="btn danger" id="attackSentinel">Run adversarial self-test</button><button class="btn" id="decisionReplay">Decision history</button></div></details></div>`;
  }
  ...
  async function loadSecurity(showStatus=true){
    try{
      ...
      if(state.page==='governance'||state.page==='controls')renderShell();
  ...
  async function updateSourcePolicy(key,config){
    try{
      ...
      closeSheet(); state.security=null; state.overviewAnswer=null;
      const enabled=(r.details||{}).retrieval_enabled!==false;
      state.boundaryEvent={key,name:r.name||key,enabled,at:new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'})};
      await Promise.all([loadSecurity(false),loadManagerControls(false)]); renderShell();
      status(`${r.name} ${enabled?'included in':'removed from'} governed retrieval · next answer recomputes live`,'ok',3600);
    }catch(e){status(e.message,'error',3200);}
  }
  ...
  async function runCustomerExport(mode){
    ...
    try{
      ...
      if(!response.ok){
        ...
        const detail=typeof d.detail==='string'?d.detail:(d.detail?.message||`Backend denied the ${mode} export.`);
        state.lastSecurityAction={mode,outcome:'BLOCKED',http:response.status,label:`${mode.toUpperCase()} EXPORT BLOCKED`,message:detail};
        state.security=null; await loadSecurity(false); renderShell(); status(`Backend blocked ${mode} export · HTTP ${response.status}`,'error',3600); return;
      }
      ...
      state.lastSecurityAction={mode,outcome:'ALLOWED',http:response.status,label:`${mode.toUpperCase()} EXPORT ALLOWED`,message:'Authorised export completed and the action was written to the audit trail.'};
      state.security=null; await loadSecurity(false); renderShell(); status(`${cap(mode)} customer export authorised and audited`,'ok',3200);
    }catch(e){
      state.lastSecurityAction={mode,outcome:'BLOCKED',http:'ERR',label:`${mode.toUpperCase()} EXPORT BLOCKED`,message:e.message||'Request failed safely.'};
      state.security=null;await loadSecurity(false);renderShell();status(e.message,'error',3600);
    }
  }

  function renderEvidence(){
    const d=state.lastChallenge;
    return `<div class="page compact-page evidence-finals-page"><section class="page-hero reveal"><div><span class="eyebrow">JUDGE CHALLENGE MODE</span><h2>Give JurisTwin something it has never seen.</h2><p>Type an instruction or drop a file. The runtime will classify it, verify the policy collision, apply source authority and trace operational impact.</p></div><div class="hero-actions"><span class="chip cyan">LIVE INPUT</span></div></section><section class="judge-challenge-paths reveal"><article class="surface evidence-compose challenge-type-card"><div class="challenge-mode-head"><span>01</span><div><small>TYPE SOMETHING</small><h3>Judge enters a new instruction</h3></div></div><label><span class="field-label">UNSEEN MESSAGE</span><textarea class="evidence-text" id="challengeBody">Effective immediately, bank statements are no longer accepted as income proof. Officers must request payslips from gig workers.</textarea></label><div class="form-row"><input class="form-control" id="challengeSource" value="Judge Live Input" aria-label="Evidence source"><select class="form-control" id="challengeAuthority"><option value="2">Operational message</option><option value="3">Team owner</option><option value="4">Senior management</option><option value="5">Official policy owner</option></select></div><div class="compose-actions"><span class="muted">New evidence has zero publication authority by itself.</span><button class="btn primary" id="runChallenge">Analyse live ${icon('arrow',12)}</button></div></article><article class="surface challenge-file-card" id="dropZone"><div class="challenge-mode-head"><span>02</span><div><small>DROP A FILE</small><h3>Challenge with real evidence</h3></div></div><div class="challenge-drop-target">${icon('upload',24)}<b>Drop TXT · EML · JSON · CSV here</b><span>Offline deterministic parser · no internet required</span><button class="btn" type="button" id="chooseEvidenceFile">Choose file</button></div><input type="file" id="fileInput" accept=".txt,.md,.csv,.json,.eml,.log" hidden></article></section>${d?renderChallengeResult(d):renderEmptyEvidenceResult()}<section class="surface memory-panel reveal compact-memory"><div class="memory-head"><b>Current governed evidence</b><span>${state.evidence.length||'—'} sources</span></div><div class="memory-list">${state.evidence.length?state.evidence.slice(0,4).map(e=>`<div class="memory-item"><b>${esc(e.title)}</b><span>${esc(e.source)} · ${esc(e.version||'current')}</span></div>`).join(''):`<div class="memory-item"><b>Loading…</b></div>`}</div></section></div>`;
  }

  function renderEmptyEvidenceResult(){ return `<article class="surface result-stage empty-result reveal compact-empty challenge-ready"><div><div class="empty-icon">${icon('spark',20)}</div><span class="eyebrow">READY FOR JUDGE INPUT</span><h3>No prepared answer is required.</h3><p>Whatever arrives next is evaluated against the governed state at runtime.</p></div></article>`; }
  ...
  function renderChallengeResult(d){
    ...
    const verdictLabel=d.verdict==='CONTRADICTION'?'CONFLICT DETECTED':d.verdict==='NEEDS_REVIEW'?'NEEDS HUMAN REVIEW':'ALIGNED';
    const liveTime=new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'});
    const model=d.analysis?.classification||d.analysis?.hybrid_classification||{};
    const stages=(d.analysis?.stages||[]).slice(0,5);
    const pipeline=stages.length?stages.map((x,i)=>`${i?'<i>→</i>':''}<span class="done">${icon('check',11)} ${esc(String(x.name||'Runtime stage').replace('Hybrid AI policy classification','Policy AI').replace('Canonical evidence retrieval','Source retrieval').replace('Contradiction analysis','Symbolic check').replace('BFS dependency traversal','Impact graph').replace('Governed evidence ingestion','Quarantine'))}<small>${Number(x.latency_ms||0).toFixed(1)} ms</small></span>`).join(''):`<span class="done">${icon('check',11)} Runtime pipeline complete</span>`;
    return `<article class="surface result-stage reveal compact-result challenge-result-finals"><div class="live-input-stamp"><span class="runtime"><i></i><b>LIVE INPUT · RECEIVED ${liveTime}</b></span><span>${esc(d.challenge_ref||'Runtime challenge')}</span></div><div class="challenge-verdict-stage ${d.verdict==='CONTRADICTION'?'danger':d.verdict==='NEEDS_REVIEW'?'review':'safe'}"><div><small>RUNTIME VERDICT</small><h3>${verdictLabel}</h3><p>${esc(brief(plain.headline||`${fmt(d.blast_radius)} customer cases traced.`,115))}</p></div><div class="challenge-impact-number"><strong>${fmt(d.blast_radius)}</strong><span>CASES TRACED</span></div></div><div class="live-pipeline-strip">${pipeline}</div><div class="judge-message-compare compact-judge-compare"><article><small>NEW · ${esc(plain.incoming_source||d.source)}</small><blockquote>“${esc(brief(plain.what_incoming_says||d.body||'',135))}”</blockquote></article><div class="compare-arrow">VS</div><article><small>GOVERNED · ${esc(plain.canonical_source||d.analysis?.canonical?.source||'Approved evidence')}</small><blockquote>“${esc(brief(plain.what_canonical_says||d.analysis?.canonical?.claim||'',135))}”</blockquote></article></div><div class="plain-answer-grid challenge-answers compact-challenge-answers"><div><small>ACTION</small><p>${esc(brief(plain.which_source_wins||'Current approved source stays in force.',105))}</p></div><div><small>PUBLICATION</small><p>${d.verdict==='CONTRADICTION'?'Incoming evidence quarantined. Human approval is required before company truth can change.':d.verdict==='NEEDS_REVIEW'?'No automated decision. Escalated for human review.':'Evidence aligned; no conflicting policy change detected.'}</p></div></div><details class="technical-details compact-details"><summary>Why did the runtime decide this?</summary><p>${esc(brief(plain.why_conflict||collision.explanation||'The instructions were evaluated by learned routing plus deterministic policy comparison.',200))}</p></details><div class="result-footer"><span class="chip ${d.verdict==='CONTRADICTION'?'red':d.verdict==='NEEDS_REVIEW'?'amber':'green'}">${d.verdict==='CONTRADICTION'?'QUARANTINED':d.verdict==='NEEDS_REVIEW'?'ABSTAINED':'CHECK COMPLETE'}</span><button class="btn quiet" id="challengeReasoning">Why?</button><button class="btn quiet" id="challengeAIModel">Technical</button><button class="btn quiet" id="challengeToConflict" style="margin-left:auto">Source trail ${icon('arrow',12)}</button></div></article>`;
  }
  ...
  async function runChallenge(){
    const body=$('#challengeBody')?.value.trim(); if(!body||body.length<8){status('Enter at least 8 characters of policy evidence','error');return;} if(body.length>5000){status('Keep live evidence under 5,000 characters','error');return;}
    const btn=$('#runChallenge'); state.challengeStartedAt=Date.now(); if(btn){btn.disabled=true;btn.textContent='Running live pipeline…';}
    try {
  ...
  async function handleEvidenceFile(file){
    if(!file)return; const allowed=['txt','md','csv','json','eml','log']; const ext=(file.name.split('.').pop()||'').toLowerCase(); if(!allowed.includes(ext)){status('Unsupported file type','error');return;} if(file.size>150000){status('File is too large for the safe live parser (150 KB max)','error',3600);return;}
  ...
  function wirePage(){
    if(state.page==='overview'){
      ...
      $$('[data-overview-role]').forEach(b=>b.addEventListener('click',()=>runOverviewAnswer(b.dataset.overviewRole))); $('#overviewRoleProof')?.addEventListener('click',()=>runOverviewAnswer(state.overviewRole==='intern'?'manager':'intern'));
    ...
    if(state.page==='controls'){
      $('#refreshManagerControls')?.addEventListener('click',async()=>{await Promise.all([loadManagerControls(false),loadSecurity(false)]);status('Management evidence boundary re-checked','ok');});
      $$('[data-source-config]').forEach(b=>b.addEventListener('click',()=>showSourceConfig(b.dataset.sourceConfig)));
      $$('[data-source-toggle]').forEach(b=>b.addEventListener('click',()=>updateSourcePolicy(b.dataset.sourceToggle,{retrieval_enabled:b.dataset.sourceEnabled!=='true'})));
      $('#rerunBoundary')?.addEventListener('click',()=>navigate('overview',()=>setTimeout(()=>runOverviewAnswer('manager'),100)));
      $('#controlsToConflict')?.addEventListener('click',()=>{state.selectedConflict='CF-INCOME-001';navigate('conflict');});
      $('#controlsAssuranceDetails')?.addEventListener('click',()=>{$('.controls-evidence-details')?.setAttribute('open','');$('.controls-evidence-details')?.scrollIntoView({behavior:'smooth',block:'center'});});
      if(!state.managerControls)setTimeout(()=>loadManagerControls(false),60);
      if(!state.security)setTimeout(()=>loadSecurity(false),80);
    }
    ...
    if(state.page==='assurance'){
      $('#loadAssurance')?.addEventListener('click',loadAssurance);$('#refreshAssurance')?.addEventListener('click',loadAssurance);$('#proofPack')?.addEventListener('click',showProofPack);$('#attackSentinel')?.addEventListener('click',attackSentinel);$('#decisionReplay')?.addEventListener('click',showDecisionReplay);
      $('#assuranceApprove')?.addEventListener('click',publishDecision);$('#assuranceVerify')?.addEventListener('click',showProofPack);$('#assuranceNextChallenge')?.addEventListener('click',()=>navigate('evidence',()=>setTimeout(()=>$('#challengeBody')?.focus(),90)));$('#assuranceCompare')?.addEventListener('click',()=>navigate('twin'));
    ...
    if(state.page==='evidence'){
      ...
      const dz=$('#dropZone'),fi=$('#fileInput');$('#chooseEvidenceFile')?.addEventListener('click',e=>{e.stopPropagation();fi?.click();});dz?.addEventListener('click',e=>{if(e.target.closest('button,input'))return;fi?.click();});fi?.addEventListener('change',()=>handleEvidenceFile(fi.files?.[0]));dz?.addEventListener('dragover',e=>{e.preventDefault();dz.classList.add('dragover');});dz?.addEventListener('dragleave',()=>dz.classList.remove('dragover'));dz?.addEventListener('drop',e=>{e.preventDefault();dz.classList.remove('dragover');handleEvidenceFile(e.dataTransfer?.files?.[0]);});ensureEvidence();
    }
  }
