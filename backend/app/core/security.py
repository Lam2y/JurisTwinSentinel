...
def optional_current_user(creds: HTTPAuthorizationCredentials | None = Depends(bearer), db: Session = Depends(get_db)) -> User | None:
    """Resolve a session without turning a missing/stale browser token into a noisy 401.

    Protected endpoints still use ``current_user``. This helper exists only for passive
    session discovery on the finals shell so startup remains clean and observable.
    """
    if not creds:
        return None
    settings = get_settings()
