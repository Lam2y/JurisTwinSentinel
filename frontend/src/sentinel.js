(() => {
  ...
  function platformMenu(){
    openSheet({title:'JurisTwin platform',subtitle:'Pitch-deck promises + championship assurance, all live',wide:true,body:`
      ...
      ${platformGroup('CHAMPIONSHIP ASSURANCE','v4 controls that turn the prototype into a defensible enterprise PoC.',[
        ...
        capabilityCard('reasoner','Policy Reasoner','EXPLAINABILITY','Show the structured PERMITTED ↔ PROHIBITED collision behind a contradiction.','bolt'),
  ...
  async function openMemoryCapability(){
    ...
    openSheet({title:'Secure Enterprise Memory',subtitle:'Connect by meaning. Reveal by permission.',wide:true,body:`
      ...
      <div id="memoryResults" class="feature-list">${initial}</div>`,onOpen:()=>{
        ...
        $$('[data-role-preview]',portal).forEach(b=>b.addEventListener('click',()=>{preview=b.dataset.rolePreview;$$('[data-role-preview]',portal).forEach(x=>x.classList.toggle('active',x===b));run();}));
  ...
  async function openLedgerCapability(){
    ...
    const contract=decisions.find(d=>d.decision_ref==='JT-084');
    let detail=null; if(contract){try{detail=await api('/ledger/decisions/JT-084');}catch{}}
    ...
    const contractHtml=detail?`<section class="decision-contract"><div><span class="eyebrow">DECISION JT-084 · ${esc(detail.decision.version)}</span><h3>${esc(detail.decision.approved_rule)}</h3><p>Approved by <b>${esc(detail.decision.approved_by)}</b> · supersedes ${esc(detail.decision.supersedes||'legacy rule')}</p></div><div class="contract-impact"><span><b>${detail.decision.affected?.applications||27}</b> applications</span><span><b>${detail.decision.affected?.qa_tests||8}</b> tests</span><span><b>${detail.decision.affected?.tasks||3}</b> tasks</span><span><b>${detail.decision.affected?.documents||2}</b> direct documents</span></div><div class="decision-lifecycle"><button data-ledger-action="compare">Compare</button><button data-ledger-action="approve">Approve</button><button data-ledger-action="merge">Merge</button><button data-ledger-action="rollback">Roll Back</button><button data-ledger-action="restore">Restore</button></div></section><div class="version-ribbon">${detail.versions.slice().reverse().map(v=>`<span class="${v.status==='active'?'active':''}">${esc(v.version)} · ${esc(cap(v.change_type))}</span>`).join('')}</div>`:`<div class="decision-contract empty"><div><span class="eyebrow">DECISION JT-084</span><h3>Not published yet.</h3><p>Run the Digital Twin and approve Option C to create the version-controlled decision contract.</p></div><button class="btn primary" id="ledgerGoTwin">Create JT-084</button></div>`;
  ...
  function openReasonerCapability(){
    const d=state.lastChallenge,reason=d?.analysis?.policy_atoms?.reasoning||{},collisions=reason.collisions||[];
    openSheet({title:'Policy Reasoner',subtitle:'White-box policy atom collision',wide:true,body:d?`<div class="platform-intro compact"><span class="eyebrow">STRUCTURED REASONING</span><h3>${esc(d.verdict)} · ${Math.round(Number(d.confidence||0)*100)}% confidence</h3><p>Natural-language evidence is decomposed into structured policy atoms before comparison.</p></div><div class="reasoner-grid">${collisions.map(c=>`<div class="reasoner-collision"><div><small>CANONICAL</small><b>${esc(c.object||'POLICY')} · ${esc(c.canonical_modality||'')}</b></div><span>↔</span><div><small>INCOMING</small><b>${esc(c.object||'POLICY')} · ${esc(c.incoming_modality||'')}</b></div><p>${esc(c.explanation||'Explicit policy modality collision detected.')}</p></div>`).join('')||'<div class="feature-empty">No explicit collision in the latest challenge.</div>'}</div><div class="feature-actions"><button class="btn primary" id="reasonerEvidence">Open Evidence Lab</button></div>`:`<div class="feature-empty"><b>No live policy challenge yet.</b><br>Give JurisTwin unseen evidence first, then return here to inspect the extracted atoms.</div><div class="feature-actions"><button class="btn primary" id="reasonerEvidence">Challenge Sentinel</button></div>`,onOpen:()=>$('#reasonerEvidence')?.addEventListener('click',()=>{closeSheet();navigate('evidence',()=>setTimeout(()=>$('#challengeBody')?.focus(),80));})});
  }

  async function showRolloutPlan(){
    const d=await api('/assurance/rollout-plan/CF-INCOME-001');
    openSheet({title:'Progressive rollout',subtitle:'Do not make the customer the test environment',wide:true,body:`<div class="platform-intro compact"><span class="eyebrow">SAFE DELIVERY</span><h3>CANARY → CONTROLLED → FULL</h3><p>${d.affected_cases} affected cases are deterministically assigned to rollout waves. Rollback conditions remain visible before deployment.</p></div><div class="rollout-waves">${d.waves.map(w=>`<article><span class="chip ${w.name==='CANARY'?'amber':w.name==='FULL'?'green':'cyan'}">${esc(w.name)}</span><b>${w.case_count}</b><small>${esc(w.entry_criteria||'governed cohort')}</small></article>`).join('')}</div><div class="sheet-section"><h4>Rollback conditions</h4><div class="compact-tags">${[...new Set((d.waves||[]).map(x=>x.rollback_trigger).filter(Boolean))].map(x=>`<span>${esc(x)}</span>`).join('')}</div></div>`});
  }

  async function showDecisionReplay(){
    try{const d=await api('/assurance/replay/JT-084');openSheet({title:'Decision Replay',subtitle:'Reconstruct JT-084 from evidence to propagation',wide:true,body:`<div class="platform-intro compact"><span class="eyebrow">DECISION TIME MACHINE</span><h3>${esc(d.current?.version||'JT-084')} · ${esc(d.status)}</h3><p>The current decision can be reconstructed from version history and the tamper-evident ledger.</p></div><div class="replay-timeline">${(d.timeline||[]).map(x=>`<div class="replay-step"><i></i><div><b>${esc(x.label)}</b><small>${esc(x.actor||'system')} · ${esc(x.at||'')}</small></div></div>`).join('')}</div><div class="feature-actions"><button class="btn primary" id="replayLedger">Open ledger</button></div>`,onOpen:()=>$('#replayLedger')?.addEventListener('click',openLedgerCapability)});}catch(e){openSheet({title:'Decision Replay',subtitle:'Publish JT-084 first',body:`<div class="feature-empty"><b>The time machine starts after governance.</b><br>Run the Digital Twin and publish Option C, then replay the entire decision history.</div><div class="feature-actions"><button class="btn primary" id="replayTwin">Go to Digital Twin</button></div>`,onOpen:()=>$('#replayTwin')?.addEventListener('click',()=>{closeSheet();navigate('twin');})});}
  }
  ...
  async function resetDemo(){
    try {
      ...
      closeSheet(); state.sim=null; state.lastChallenge=null; state.selectedOption='C'; state.graphPositions={}; state.assurance=null; state.readiness=null; await refreshCore(); renderShell(); status('Finals scenario restored','ok');
    } catch(e) { status(e.message,'error',3200); }
  }
  ...
  function renderOverview(){
    const d=state.dashboard||{}, m=d.metrics||{}, conflicts=d.priority_conflicts||[], hero=conflicts[0]||{}, i=d.integrity||{};
    ...
    const heroTitle = resolved ? `${hero.affected_customers || 27} customers are now protected.` : `${hero.affected_customers || m.customers_at_risk || 27} customers exposed by one contradiction.`;
    ...
    return `<div class="page">
      ...
      <section class="command-grid">
        <article class="surface focus-panel reveal"><div class="focus-top"><span class="chip ${resolved?'green':'red'}">${resolved?'RESOLVED':'CRITICAL'}</span><span class="muted" style="font-size:11px">${esc(hero.conflict_ref||'CF-INCOME-001')}</span></div><h3>${esc(hero.name||'Income-document eligibility')}</h3><p>${esc(flagship.root_cause||'Approved bank-statement policy conflicts with stale payslip-only operational guidance.')}</p><div class="impact-line"><div><b>${fmt(hero.affected_customers||flagship.affected_customers||27)}</b><span>customers</span></div><div><b>${fmt(hero.systems_affected||flagship.systems_affected||5)}</b><span>systems</span></div><div><b>${Math.round(Number(flagship.confidence||.942)*100)}%</b><span>confidence</span></div></div><button class="btn focus-cta" id="overviewTwin">${resolved?'Inspect decision':'Run Digital Twin'} ${icon('play',12)}</button><div class="story-rail six"><div class="story-step done">Connect</div><div class="story-step done">Expose</div><div class="story-step ${state.sim||resolved?'done':''}">Simulate</div><div class="story-step ${state.sim||resolved?'done':''}">Recommend</div><div class="story-step ${resolved?'done':''}">Approve</div><div class="story-step ${resolved?'done':''}">Protect</div></div></article>
        ...
        <section class="surface queue-panel reveal"><div class="queue-head"><div><b>Priority queue</b><span> · only what needs attention</span></div><span>${conflicts.length} open</span></div>${conflicts.slice(0,3).map(c=>`<button class="queue-row" data-open-conflict="${esc(c.conflict_ref)}" type="button" style="border-left:0;border-right:0;border-bottom:0;background:transparent;color:inherit;width:100%;text-align:left;cursor:pointer"><div><b>${esc(c.name)}</b><small>${esc(c.conflict_ref)}</small></div><span class="count">${fmt(c.affected_customers)} affected</span><span class="chip ${String(c.severity).toLowerCase()==='critical'?'red':String(c.severity).toLowerCase()==='high'?'amber':'cyan'}">${esc(c.severity)}</span></button>`).join('')}</section>
      </section>
    </div>`;
  }
  ...
  function renderTwinResults(sim){
    ...
    const names={A:'Take No Action',B:'Update the FSD Only',C:'Align the Complete Process'};
    const risk={A:'High',B:'Medium',C:'Low'};
    return `<div class="twin-summary reveal"><span class="chip green">WHITE-BOX · ROBUST</span><span><strong>${fmt(sim.analysis?.scenario_count||1500)}</strong> scenarios · sensitivity + uncertainty + Pareto checks visible</span></div><section class="decision-lanes reveal">${sim.options.map(o=>`<article class="decision-lane ${state.selectedOption===o.key?'selected':''} ${sim.recommended_option===o.key?'recommended':''}" data-option="${esc(o.key)}" tabindex="0"><div class="lane-key">Option ${esc(o.key)}</div><h3>${esc(names[o.key]||cap(o.name))}</h3><div class="fit-score">${Number(o.decision_fit).toFixed(1)}<small>/100 fit</small></div><div class="fit-bar"><i style="width:${clamp(Number(o.decision_fit),0,100)}%"></i></div><div class="lane-metrics"><div><b>${Number(o.predicted_delay_days).toFixed(1)}d</b><span>delay</span></div><div><b>${risk[o.key]}</b><span>${pct(o.complaint_probability)} complaint</span></div><div><b>${pct(o.policy_alignment)}</b><span>alignment</span></div></div>${o.key==='B'?'<div class="lane-gap">Remaining gap: officers not notified</div>':''}${o.key==='C'?'<div class="action-chips"><span>Update FSD</span><span>Notify officers</span><span>Review cases</span><span>Block duplicates</span></div>':''}</article>`).join('')}</section><section class="twin-bottom"><article class="surface certificate-panel reveal"><div class="cert-kicker">RECOMMENDED ACTION · WHY OPTION C?</div><h3>Align the complete process — not just one document.</h3><p>${esc(sim.analysis?.recommended_rationale||'Option C resolves both the document conflict and its operational consequences, and remains the strongest option as assumptions move.')}</p><div class="cert-strip"><div><b>${fmt(sim.analysis?.scenario_count||1500)}</b><span>scenarios</span></div><div><b>${pct(cert.sensitivity_stability_pct||sim.analysis?.robustness_score)}</b><span>stable tests</span></div><div><b>${Number(cert.worst_case_fit_p10||0).toFixed(1)}</b><span>p10 fit</span></div></div></article><aside class="surface weights-panel reveal"><h4>Decision priorities</h4><p>Transparent weights. No hidden model preference.</p>${weightRows()}</aside></section>`;
  }
  ...
  async function runTwin(){
    try {
      ...
      state.sim=await api('/simulations/conflict/CF-INCOME-001/run',{method:'POST',body:JSON.stringify({weights})}); state.selectedOption=state.sim.recommended_option||'C'; renderShell(); status('Decision certificate issued','ok');
    } catch(e){ status(e.message,'error',3200); }
  }
  ...
  async function publishDecision(){
    ...
    try {
      ...
      closeSheet(); await refreshCore(); state.page='overview'; renderShell(); status(`Decision ${result.decision_contract?.decision_ref||'published'} · ${result.propagation?.cases||0} cases protected`,'ok',3600);
  ...
  async function showProofPack(){
    openSheet({title:'Decision assurance proof',subtitle:'Signed, replayable evidence dossier',body:`<div class="sheet-section"><p>Building the proof pack from live evidence, reasoning, impact and ledger state…</p></div>`});
    try {
      const d=await api('/assurance/proof-pack'); const proof=d.proof||{}, ledger=d.ledger||{}, impact=d.impact||{};
      $('.sheet-body',portal).innerHTML=`<div class="sheet-section"><span class="chip green">${esc(d.status||'ASSURED')}</span><h4 style="font-size:21px;margin-top:15px">One verifiable decision dossier.</h4><p>Evidence, reasoning, impact and ledger posture are fingerprinted together and authenticated for later verification.</p></div><div class="sheet-section"><dl class="sheet-kv"><dt>Blast radius</dt><dd>${impact.affected_cases||0} cases · ${impact.reachable_nodes||0} graph nodes</dd><dt>Ledger</dt><dd>${ledger.verified?'VERIFIED':'CHECK'} · ${ledger.entries||0} chained events</dd><dt>Digest</dt><dd class="mono">${esc(shortHash(proof.bundle_digest))}</dd><dt>Signature</dt><dd class="mono">${esc(shortHash(proof.signature))}</dd><dt>Algorithm</dt><dd>${esc(proof.signature_algorithm||'HMAC-SHA256')}</dd><dt>Key</dt><dd>${esc(proof.key_id||'juristwin-assurance')}</dd></dl></div>`;
  ...
  function wirePage(){
    if(state.page==='overview'){
      $('#openCritical')?.addEventListener('click',()=>navigate(state.conflicts.find(c=>c.conflict_ref==='CF-INCOME-001')?.status==='resolved'?'assurance':'conflict'));
      $('#overviewTwin')?.addEventListener('click',()=>{if(state.conflicts.find(c=>c.conflict_ref==='CF-INCOME-001')?.status==='resolved')navigate('assurance');else navigate('twin',()=>setTimeout(runTwin,60));});
    ...
    if(state.page==='conflict'){
      $$('[data-conflict]').forEach(b=>b.addEventListener('click',()=>{state.selectedConflict=b.dataset.conflict;state.selectedNode=null;renderShell();}));
    ...
    if(state.page==='evidence'){
      $('#runChallenge')?.addEventListener('click',runChallenge);$('#challengeToConflict')?.addEventListener('click',()=>navigate('conflict'));
